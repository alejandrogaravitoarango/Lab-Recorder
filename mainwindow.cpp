...
QMainWindow(parent),
ui(new Ui::MainWindow),outlet(lsl::stream_info("Length","Duration",1,0,lsl::cf_string,"MP-2010.08.01")) {
...
//	QObject::connect(ui->actionLoadConfig, SIGNAL(triggered()), this, SLOT(browseConfigDialog()));
    ...
    connect(timer1, SIGNAL(timeout()), this, SLOT(statusLength()));
    timer1->start(1000);

}

void MainWindow::statusLength(void) {
...
void MainWindow::refreshStreams(void) {
	...
	for(unsigned i=0; i<missingStreams.size(); i++) {
        ...
        for(int i=0;i<ui->streamList->count();i++){
            // automatically check and hide the Length stream
            if(ui->streamList->item(i)->text().toStdString().compare("Length (wfs)")==0){
                (ui->streamList->item(i)->setCheckState(Qt::Checked));
                (ui->streamList->setRowHidden(i,true));
            }
        }
